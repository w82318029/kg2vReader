...
class UtteranceDecoder(EncoderDecoderBase):
    ...
    def __init__(self, state, rng, parent, dialog_encoder, word_embedding_param):
        ...
        ####################################
        #self.menu_top200_decoder = menu_top200_decoder
        #########################################
        self.init_params()
    ...
    def init_params(self): 
        ...
        '''
        # yawa add attention variables   + self.sdim
        self.U_att = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, self.qdim_decoder*3  , self.qdim_decoder), name='U_att'))
        self.W_att = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, self.qdim_decoder, self.qdim_decoder), name='W_att'))
        self.v_att = add_to_params(self.params, theano.shared(0.2 * numpy.random.uniform(-1.0, 1.0,(self.qdim_decoder)).astype(theano.config.floatX), name='v_att')) 
        self.Emb_tune = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, self.rankdim, self.qdim_decoder), name='Emb_tune'))
        self.Emb_out = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, self.qdim_decoder*3  , out_target_dim), name='Emb_out'))
    ...
    def build_output_layer(self, hs, xd, hd):
        ...
        return pre_activ
    ##yawa add def build_output_layer_attention
    
    def build_output_layer_attention_h_bi(self, hs, xd, hd, h, att_mask):
        if self.utterance_decoder_gating == "LSTM":
            if hd.ndim != 2:
                pre_activ = T.dot(hd[:, :, 0:self.qdim_decoder], self.Wd_out)
            else:
            ...
            pre_activ = T.dot(hd, self.Wd_out)
        ## yawa
        xt_tensor = tensor.tensordot(xd, self.Emb_tune, (2,0))
        scope_tensor = T.concatenate([xt_tensor, h], axis=2)
        #scope_tensor = h
        scope_U_tensor = T.tensordot(scope_tensor, self.U_att, (2,0))
        if self.utterance_decoder_gating == "LSTM":
            if hd.ndim != 2:
                htm1_W_tensor = T.dot(hd[:, :, 0:self.qdim_decoder], self.W_att)
            else:
                htm1_W_tensor = T.dot(hd[:, 0:self.qdim_decoder], self.W_att)
        else:
            htm1_W_tensor = T.dot(hd, self.W_att)
        #htm1_W_tensor = T.tensordot(hd, self.W_att, (2,0))
        def att_slice(scope_cube_partial, scope_U_partial, htm1_W_mat, v_att, att_mask_slice):
            # the scope U is already multiplied by U_att, and htm1_mat already multiplied by W_att
            # the scope is not multiplied by U_att
            beta_3 = T.tanh(scope_U_partial+htm1_W_mat)
            beta_4 = T.tensordot(beta_3, v_att, (2,0))
            alpha_mat = T.nnet.softmax(T.transpose(beta_4, axes=(1,0)))
            ###add att_mask 
            alpha_mat = alpha_mat* att_mask_slice.T
            #print "eval:", eval(alpha_mat)
            alpha_mat = alpha_mat / (alpha_mat.sum(1,keepdims=True)+0.00001)
            ##########
            
            new_h_cube = T.transpose(scope_cube_partial, axes=(1,0,2))
            zt_mat = T.sum((alpha_mat[:,:,None]*new_h_cube), axis=1)
            return zt_mat
        def att_func(slice_idx, scope_cube, scope_U, htm1_W_cube, v_att, att_mask):
            att_slicea = att_mask[slice_idx]
            #scope_cube_partial = (scope_cube.dimshuffle(2,0,1) * att_slicea).dimshuffle(1,2,0)
            #scope_U_partial = (scope_U.dimshuffle(2,0,1) * att_slicea).dimshuffle(1,2,0)
            scope_cube_partial = scope_cube * att_slicea[:,:,None]
            scope_U_partial = scope_U * att_slicea[:,:,None]
            #scope_cube_partial = T.set_subtensor(scope_cube, att_slicea[:,:,None])
            #scope_U_partial = T.set_subtensor(scope_cube, att_slicea[:,:,None])
            htm1_W_mat = htm1_W_cube[slice_idx,:,:]
            zt_mat = att_slice(scope_cube_partial, scope_U_partial, htm1_W_mat, v_att, att_mask[slice_idx])
            return zt_mat
        def att_out(zt_cube, ht_cube, Emb_out):
            pre_tran = tensor.concatenate([zt_cube, ht_cube], axis=2)
            post_tran = tensor.tensordot(pre_tran, Emb_out, (2,0))
            #yt_mat = tensor.nnet.softmax( pre_yt_mat )
            #logyt_mat = T.log( yt_mat + np.float32(1e-8) )
            #
            return post_tran
        zt_tensor, _ = theano.scan(
            fn=att_func,
            sequences=[T.arange(htm1_W_tensor.shape[0])], #T.arange(htm1_W_tensor.shape[0])
            outputs_info=None,
            non_sequences=[scope_tensor, scope_U_tensor, htm1_W_tensor, self.v_att, att_mask])
        pre_activ += T.dot(zt_tensor, self.Emb_out) 
        ###
        '''
        if self.deep_out:
            pre_activ += T.dot(xd, self.Wd_e_out) + self.bd_e_out
    ...
    ##yawa add def build_output_layer_attention_beam_search_h_hs
    
    def build_output_layer_attention_beam_search_h_bi(self, hs, xd, hd, h):
        if self.utterance_decoder_gating == "LSTM":
            if hd.ndim != 2:
                pre_activ = T.dot(hd[:, :, 0:self.qdim_decoder], self.Wd_out)
            else:
        ...
        def att_slice(scope_cube_partial, scope_U_partial, htm1_W_mat, v_att):
            # the scope U is already multiplied by U_att, and htm1_mat already multiplied by W_att
            # the scope is not multiplied by U_att
            beta_3 = T.tanh(scope_U_partial+htm1_W_mat)
            beta_4 = T.tensordot(beta_3, v_att, (2,0))
            ...
            zt_mat = T.sum((alpha_mat[:,:,None]*new_h_cube), axis=1)
            return zt_mat
        def att_func(slice_idx, scope_cube, scope_U, htm1_W_cube, v_att):  #slice_idx,
            scope_cube_partial = scope_cube#[:slice_idx+1,:,:]
            scope_U_partial = scope_U#[:slice_idx+1, :, :]
            htm1_W_mat = htm1_W_cube#[slice_idx,:,:]
            zt_mat = att_slice(scope_cube_partial, scope_U_partial, htm1_W_mat, v_att)
            return zt_mat
        def att_out(zt_cube, ht_cube, Emb_out):
            pre_tran = tensor.concatenate([zt_cube, ht_cube], axis=2)
            post_tran = tensor.tensordot(pre_tran, Emb_out, (2,0))
        ...
        zt_tensor, _ = theano.scan(
            ...
            outputs_info=None,
            non_sequences=[scope_tensor, scope_U_tensor, htm1_W_tensor, self.v_att])
        pre_activ += T.dot(zt_tensor[0], self.Emb_out) 
        
        #zt_tnesor,_= att_func(scope_tensor, scope_U_tensor, htm1_W_tensor, self.v_att)
    ...
    def build_next_probs_predictor(self, inp, x, h, hs_raw, x_raw, prev_state):  #yawa add h, hs_raw, xd_raw
        """ 
        ...
        """
        return self.build_decoder(inp, h, hs_raw,  x , x_raw = x_raw, mode=UtteranceDecoder.BEAM_SEARCH, prev_state=prev_state)  #yawa add h
    ...
    def build_decoder(self, decoder_inp, h, hs_raw, x, x_raw=None, xmask=None, xdropmask=None, y=None, y_neg=None, mode=EVALUATION, prev_state=None, step_num=None, att_mask=None):
    #yawa add h
        ...
        if x_raw!=None:
            if self.decoder_drop_previous_input_tokens and xdropmask:
                #xdropmask = xdropmask.dimshuffle(0, 1, 'x')
                xd_raw2 = xdropmask*self.approx_embedder(x_raw) + (1-xdropmask)*self.word_embedding_param[self.unk_sym].dimshuffle('x', 'x', 0)
            else:
                #print x_raw.eval()
                xd_raw2 = self.approx_embedder(x_raw)
        ...
        '''
        ##for bi herd output attention ###############
        ones_vector = T.ones_like(xmask[0,:]).dimshuffle('x', 0)
        rolled_xmask = T.concatenate([ones_vector, xmask], axis=0)
        def pre_dec(m_t,cnt,pre):
            idx = m_t*pre + (1-m_t)*cnt    
            pre = idx
            return pre

        cnt_list = T.arange(xmask.shape[0])
        pre_init = T.alloc(int64(0), xmask.shape[1])
        if xd.ndim!=3:
            #_pre = (T.ones_like(xd[0]))*(xd.shape[1])
            _pre = xmask.shape[0]
        else:
            _pre, _ = theano.scan(pre_dec,
                              sequences=[rolled_xmask, cnt_list[:]],\
                              outputs_info=[pre_init])
        ############################################
        '''
        ...
        step_decoder_inp = decoder_inp  ####beam search, only the previous step hs. not the entire hs.
        if self.decoder_bias_type == "selective":
            step_decoder_inp = _res[1]
        #pre_activ = self.build_output_layer(step_decoder_inp, xd, hd)
        #yawa add
        if mode == UtteranceDecoder.EVALUATION or mode == UtteranceDecoder.NCE:
            #pre_activ = self.build_output_layer_attention(step_decoder_inp, xd, hd, h)
            pre_activ = self.build_output_layer_attention_h_bi(step_decoder_inp, xd, hd, h, att_mask )
            #pre_activ = self.build_output_layer_attention_hs(step_decoder_inp, xd, hd, h)
            #pre_activ = self.build_output_layer_attention_h_hs(step_decoder_inp, xd, hd, h)
        elif mode == UtteranceDecoder.BEAM_SEARCH:
            pre_activ = self.build_output_layer_attention_beam_search_h_bi(step_decoder_inp, xd_raw2, hd, h)
            #pre_activ = self.build_output_layer_attention_beam_search_hs(step_decoder_inp, xd_raw2, hd, h, hs_raw)
            #pre_activ = self.build_output_layer_attention_beam_search_h_hs(step_decoder_inp, xd_raw2, hd, h, hs_raw)
            #pre_activ = self.build_output_layer(step_decoder_inp, xd, hd)
...
class DialogEncoderDecoder(Model):
    ...
    def build_train_function(self):
        if not hasattr(self, 'train_fn'):
            ...
            self.train_fn = theano.function(inputs=[self.x_data, self.x_data_reversed, 
                                                         ...
                                                         self.x_reset_mask, 
                                                         self.ran_cost_utterance, self.x_dropmask,self.att_mask],
    ...
    def build_eval_function(self):
        if not hasattr(self, 'eval_fn'):
            # Compile functions
            logger.debug("Building evaluation function")##yawa add att_mask
            self.eval_fn = theano.function(inputs=[self.x_data, self.x_data_reversed, self.x_max_length, self.x_cost_mask, self.x_reset_mask, self.ran_cost_utterance, self.x_dropmask,self.att_mask], 
    ...
    def build_next_probs_function(self):
        if not hasattr(self, 'next_probs_fn'):
            ...
            outputs, hd = self.utterance_decoder.build_next_probs_predictor(decoder_inp, self.beam_source, self.beam_h_raw, self.beam_hs_raw, self.beam_xd_raw, prev_state=self.beam_hd) #yawa add self.h_raw
            self.next_probs_fn = theano.function(inputs=[self.beam_hs, self.beam_hd, self.beam_source, self.beam_x_data, self.beam_ran_cost_utterance,self.beam_h_raw, self.beam_hs_raw, self.beam_xd_raw],
                outputs=[outputs, hd],
                on_unused_input='warn',
                name="next_probs_fn")
    ...
    def build_encoder_function(self):
        if not hasattr(self, 'encoder_fn'):
            ...
            if self.direct_connection_between_encoders_and_decoder:
                ...
                #yawa add self.h
                _, hd, _, _ = self.utterance_decoder.build_decoder(hd_input, self.h, hd_input, self.x_data, y=training_y, mode=UtteranceDecoder.EVALUATION, att_mask=self.att_mask)
    ...
    def __init__(self, state):
        ...
        ###yawa add att mask
        self.att_mask = T.ftensor3('att_mask')
        ...
        if self.initialize_from_pretrained_word_embeddings == True:
            ...
            #############################################################################################
            pretrained_menu = cPickle.load(open('menu_emb_np.pkl', 'r'))
            #top200menu = cPickle.load(open('top200menu.pkl', 'r'))
            
            #nemu_matrix_raw = pretrained_matrix[1]
            ##############################################################################################
            
            # Check all dimensions match from the pretrained embeddings
            assert(self.idim == pretrained_embeddings[0].shape[0])
            assert(self.rankdim/2 == pretrained_embeddings[0].shape[1])
            assert(self.idim == pretrained_embeddings[1].shape[0])
            assert(self.rankdim/2 == pretrained_embeddings[1].shape[1])
            ############################################################################
            assert(self.rankdim/2 == pretrained_menu[0].shape[1])
            assert(self.idim == pretrained_menu[0].shape[0])
            self.W_emb_pretrained_mask = theano.shared(pretrained_embeddings[1].astype(numpy.float32), name='W_emb_mask')
            self.W_emb_2 = add_to_params(self.global_params, theano.shared(value=pretrained_embeddings[0].astype(numpy.float32), name='W_emb_2'))
            self.menu_emb = add_to_params(self.global_params, theano.shared(value=pretrained_menu[0].astype(numpy.float32), name='menu_emb'))
            self.W_menu_emb = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, self.rankdim/2, self.rankdim/2), name='W_menu_emb'))
            self.b_menu_emb = add_to_params(self.params, theano.shared(value=np.zeros((self.rankdim/2,), dtype='float32'), name='b_menu_emb'))
            self.menu_emb1 = T.nnet.sigmoid(T.dot(self.menu_emb, self.W_menu_emb) + self.b_menu_emb)
            self.W_emb= T.concatenate([self.W_emb_2,self.menu_emb1], axis=1)
            
            ###top200 menu
            #self.menu_top200_raw = add_to_params(self.global_params, theano.shared(value=top200menu[0].astype(numpy.float32), name='menu_top200_raw'))
            #self.W_menu_top200 = add_to_params(self.params, theano.shared(value=NormalInit(self.rng, self.rankdim/2, self.rankdim/2), name='W_menu_top200'))
            #self.b_menu_top200 = add_to_params(self.params, theano.shared(value=np.zeros((self.rankdim/2,), dtype='float32'), name='b_menu_top200'))
            #self.menu_top200 = T.nnet.sigmoid(T.dot(self.menu_top200_raw, self.W_menu_top200) + self.b_menu_top200)
            #self.menu_top200 = T.tile(self.menu_top200_small,(self.bs,1,1)).dimshuffle(1,0,2)
            #self.menu_emb = T.nnet.sigmoid(T.dot(self.menu_emb1, self.W_menu_emb) + self.b_menu_emb)
            ##########################################
        else:
...
# * self.x_cost_mask[1:self.x_max_length]) * (T.sum(T.eq(training_x, self.eos_sym)) / (T.sum(training_x_cost_mask_flat)))
        ...
        if self.direct_connection_between_encoders_and_decoder:
            ...
            contrastive_cost, self.hd_nce = self.utterance_decoder.build_decoder(self.hd_input, self.h, self.hd_input, training_x,  y_neg=self.y_neg, y=training_y, xmask=training_hs_mask, xdropmask=training_x_dropmask, mode=UtteranceDecoder.NCE, prev_state=self.phd, att_mask=self.att_mask)

            logger.debug("Build decoder (EVAL)")##yawa add att_mask
            target_probs, self.hd, self.utterance_decoder_states, target_probs_full_matrix = self.utterance_decoder.build_decoder(self.hd_input, self.h, self.hd_input, training_x,   xmask=training_hs_mask, xdropmask=training_x_dropmask, y=training_y, mode=UtteranceDecoder.EVALUATION, prev_state=self.phd, att_mask=self.att_mask)
        ...
        #add by yawa
        self.beam_h_raw = T.ftensor3("beam_h_raw")
        self.beam_hs_raw = T.ftensor3("beam_hs_raw")
        self.beam_xd_raw = T.imatrix("beam_xd_raw")
        ####
