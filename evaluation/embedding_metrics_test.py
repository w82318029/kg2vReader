...
def average(fileone, filetwo, w2v):
    ...
    for i in range(len(r1)):
        tokens1 = r1[i].strip().split(" ")
        tokens2 = r2[i].split('\t')[0].strip().split(" ")
        #tokens2 = r2[i].strip().split(" ")
        X= np.zeros((dim,))
        for tok in tokens1:
            if tok in w2v:
...
if __name__ == "__main__":
    ...
    raw_dict = cPickle.load(open('/data/wangyanmeng/icassp/hed-dlg-truncated-master/Data/menu/menu1.8wanci.dict.pkl', 'r'))
    str_to_idx = dict([(tok, tok_id) for tok, tok_id, _, _ in raw_dict])
    w2v = load_bin_vec("/home/wangyanmeng/neroComputiing/nlpccData/GoogleNews-vectors-negative300.bin", str_to_idx)
    #w2v = Word2Vec.load_word2vec_format(args.embeddings, binary=True)
    ground_truth = "Data/menu/test_responses.txt"

    r = average(ground_truth, sys.argv[1],w2v)
    print(sys.argv[1]+": %f +/- %f ( %f )" %(r[0], r[1], r[2]))
